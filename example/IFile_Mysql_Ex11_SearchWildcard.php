...
/**
 * This script is a example how to search a "Wildcard" term in the index with MySqli Interface
 * 
 * IMPORTANT:
...
try {
	...
	// TEST Query Wildcard	
	// 1. Search Wildcard Term	
	// For search IFile use IFileQueryRegistry
	$ifileQueryRegistry = new IFileQueryRegistry();
	$ifileQueryRegistry->setQuery('intr*', 'body', IFileQuery::MATCH_REQUIRED);
	// define order
	$ifile->setSort('key', SORT_STRING, SORT_DESC);		
	// call Wildcard method
	$result = $ifile->queryWildcard($ifileQueryRegistry);	
	// print result
